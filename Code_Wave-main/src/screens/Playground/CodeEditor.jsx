...
const CodeEditor = ({
  ...
  currentCode,
  setCurrentCode,
  generatedCode, // Add generatedCode as a prop
}) => {
  const [theme, setTheme] = useState(githubDark);
  const [language, setLanguage] = useState(javascript);

  // Update the editor's content when generatedCode changes
  useEffect(() => {
    if (generatedCode) {
      setCurrentCode(generatedCode); // Update the currentCode state with generatedCode
    }
  }, [generatedCode, setCurrentCode]); // Add setCurrentCode to the dependency array
